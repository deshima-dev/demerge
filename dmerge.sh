...
#
# Revision History:
#    2021-08-12 - TT - run.sh, support analysis id option 
# Planed update:
#    Suport DDB_20180619.fits.gz, dfits_dict.yaml, and cabin.db options
...
if [ ! -d ${CACHE_DIR}/${ANAID} ]; then
    mkdir -p ${CACHE_DIR}/${ANAID}
    if [ $? -ne 0 ]; then
	exit 1
    fi
fi
if [ ! -d ${GRAPH_DIR}/${ANAID} ]; then
    mkdir -p ${GRAPH_DIR}/${ANAID}
    if [ $? -ne 0 ]; then
	exit 1
    fi
fi
...
ls ${CACHE_DIR}/${ANAID}/*.pkl | xargs -P${NCPU} -n1 python calc_resonance_params.py
if [ $? -ne 0 ]; then
    echo "失敗:calc_resonance_params.py"
    exit 1
fi

rm -f "${CACHE_DIR}/${ANAID}/reduced_${ANAID}.fits"
if [ $? -ne 0 ]; then
    exit 1
fi
...
python make_reduced_fits.py \
       "${CACHE_DIR}/${ANAID}" \
       "${CACHE_DIR}/${ANAID}/reduced_${ANAID}.fits"
if [ $? -ne 0 ]; then
...
python mergetofits.py \
       "${CACHE_DIR}/${ANAID}/reduced_${ANAID}.fits" \
       "${CACHE_DIR}/${ANAID}/dfits_${ANAID}.fits.gz" \
...
#
FILENAMES=""
for FILENAME in `ls ${CACHE_DIR}/${ANAID}/*.pkl`
do
    FILENAMES="${FILENAME} ${GRAPH_DIR}/${ANAID} ${FILENAMES}"
done
