...
def convert_dfits_to_dems(filename, **kwargs):
    ...
    if not (
        shuttle_min_lon_off == None
        and shuttle_max_lon_off == None
        and shuttle_min_lon_on == None
        and shuttle_max_lon_on == None
    ):
    ...
    time_skychop = np.array(
        [datetime.fromtimestamp(t) for t in skychop["time"]]
    ...
    f_scantype = interp1d(
        ...
        kind="nearest",
        bounds_error=False,
        fill_value=(scan_type_numbers[0], scan_type_numbers[-1]),
    )
    ...
    f_skychop = interp1d(
        ...
        kind="nearest",
        bounds_error=False,
        fill_value=(skychop["state"][0], skychop["state"][-1]),
    )
    ...
    if still_period != None:
        for i in range(int(seconds[-1]) // still_period + 1):
            off_mask = (still_period * (2 * i) <= seconds) & (
                seconds < still_period * (2 * i + 1)
            )
            on_mask = (still_period * (2 * i + 1) <= seconds) & (
                seconds < still_period * (2 * i + 2)
            )
            scan[off_mask] = "OFF"
            scan[on_mask] = "SCAN"
