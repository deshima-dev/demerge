...
def create_bintablehdu(hd):
    ...
    columns = [
        ...
        for (i, j, k) in zip(
            hd["col_vals"].items(), hd["col_form"].items(), hd["col_unit"].items()
        )
    ]
...
def calibrate_to_power(pixelid, Troom, Tamb, rhdus, ddb):
    ...
    linphase = np.transpose(
        [rhdus["READOUT"].data["Amp, Ph, linPh %d" % i].T[2] for i in range(nkid)]
    )
    linyfc = rhdus["KIDSINFO"].data["yfc, linyfc"].T[1]
    Qr = rhdus["KIDSINFO"].data["Qr, dQr (300K)"].T[0]
    fshift = np.array((linphase - linyfc) / (4.0 * Qr)).T
...
def retrieve_misti_log(filename):
    ...
    table = ascii.read(
        filename, guess=False, format="no_header", delimiter=" ", names=column_names
    )
    ...
    for row in table:
        datetimes.append(
            datetime.strptime(
                "{} {}".format(row["date"], row["time"]), "%Y/%m/%d %H:%M:%S.%f"
            )
        )
