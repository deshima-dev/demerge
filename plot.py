...
def plot(kid, oldkid=None):
    ...
    rewound_localsweep = dmerge.gaolinbg_rewind(
        kid["localsweep"].x, kid["localsweep"].iq, arga, absa, tau, fr, Qr, Qc, phi0, c
    )
    fitted_localsweep = dmerge.gaolinbg_function()(
        kid["localsweep"].x, arga, absa, tau, fr, Qr, Qc, phi0, c
    )
    limited_fitted_localsweep = dmerge.gaolinbg_function()(
        kid["localsweep"].x[fitrange], arga, absa, tau, fr, Qr, Qc, phi0, c
    )
    rewound_fitted_localsweep = dmerge.gaolinbg_rewind(
        kid["localsweep"].x, fitted_localsweep, arga, absa, tau, fr, Qr, Qc, phi0, c
    )
    fitted_background = dmerge.gaolinbg_bg_function()(
        kid["localsweep"].x, arga, absa, tau, fr, Qr, Qc, phi0, c
    )
    limited_fitted_background = dmerge.gaolinbg_bg_function()(
        kid["localsweep"].x[fitrange], arga, absa, tau, fr, Qr, Qc, phi0, c
    )
    rewound_tod = dmerge.gaolinbg_rewind(
        kid["tod"].frequency, kid["tod"].iq[::100], arga, absa, tau, fr, Qr, Qc, phi0, c
    )
    ...
    ax2.plot(
        np.real(limited_fitted_localsweep),
        np.imag(limited_fitted_localsweep),
        "-c",
        label="fit",
    )
    ax2.plot(np.real(fitted_peak), np.imag(fitted_peak), "r*")
    ax2.plot(
        np.real(fitted_background),
        np.imag(fitted_background),
        "-",
        color="gray",
        label="bg",
    )
    if not oldkid is None:
        ...
        ax2.plot(np.real(fitted_peak), np.imag(fitted_peak), "r*")
        ax2.plot(
            np.real(oldkid["bg"]), np.imag(oldkid["bg"]), "-", color="gray", label="bg"
        )
    ...
    ax3.plot(
        np.real(rewound_fitted_localsweep),
        np.imag(rewound_fitted_localsweep),
        "y",
        lw=3,
        alpha=0.5,
        label="fit",
    ...
    ax4.plot(
        kid["localsweep"].x, phase_fitted_localsweep, "y", lw=3, alpha=0.5, label="fit"
    )
    ...
    ts = kid["tod_timestamp"][
        :-100
    ]  # データの最後の部分にはよくないデータが含まれる可能性があるので、最後の100個ぐらいを除外する(make_reduced_fitsで処理されている)。
    ts = ts - ts[0]
    ...
    ax1.plot(
        ts,
        np.ones(len(ts)) * linyfc,
        "c--",
        lw=1,
        label="carrier linphase (from cache)",
    )
    ax1.plot(
        ts,
        np.ones(len(ts)) * yfc,
        ...
        lw=3,
        alpha=0.3,
        label="carrier phase (from cache)",
    )
    # ax1.set_ylim(-2, 2)
    ...
    ax2.plot(
        ts, np.ones(len(ts)) * linyfc, "c--", lw=1, label="carrier linphase (from fits)"
    )
    ax2.plot(
        ts,
        np.ones(len(ts)) * yfc,
    ...
    f_, ampl_ = dmerge.power_spectrum_density(
        ampl[:size], dt, 7, window=None, overwrap_half=True
    )
    f_, phase_ = dmerge.power_spectrum_density(
        phase[:size], dt, 7, window=None, overwrap_half=True
    )
    f_, linphase_ = dmerge.power_spectrum_density(
        linphase[:size], dt, 7, window=None, overwrap_half=True
    )
    ax3.semilogx(f_, np.log10(ampl_) * 10.0, "r", label="ampl. (from fits)")
    ax3.semilogx(
        f_, np.log10(linphase_) * 10.0, "b", label="linearized phase (from fits)"
    )
    ax3.semilogx(
        f_, np.log10(phase_) * 10.0, "g", lw=3, alpha=0.3, label="phase (from fits)"
    )
    ax3.set_xlabel("Frequency [Hz]")
    ax3.set_ylabel("PSD [dBk/Hz]")
    ax3.set_ylim(-110.0, -50.0)
    ax3.legend(loc="best")
    ax3.grid()
...
if __name__ == "__main__":
    ...
    if not fig1 is None:
        ...
        plt.savefig(
            "{}/sweep_kid{:05}.png".format(graph_dir, kid["kidid"]), transparent=False
        )
        plt.close()
    if not fig2 is None:
