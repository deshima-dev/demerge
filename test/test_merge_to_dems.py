...
class MergeToDemsTestDrive(unittest.TestCase):
    ...
    def test_short_measure(self):
        ...
        times_skychop, states_skychop = mf.retrieve_skychop_states(
            "{}.skychop".format(prefix)
        )
        times_skychop = mf.convert_timestamp(times_skychop)
        times_skychop = np.array(times_skychop).astype("datetime64[ns]")

        antenna_table = ascii.read("{}.ant".format(prefix))[
            :-1
        ]  # 最後の1行は終端を表す意味のないデータが入っているため無視する
        times_antenna = mf.convert_asciitime(
            antenna_table["time"], "%Y-%m-%dT%H:%M:%S.%f"
        )
        ...
        #
        self.assertEqual(
            len(dems.time.values),
            len(dems.d2_skychopper_isblocking.values),
            "d2_skychopper_isblockingの要素数の確認",
        )

        indices_extrapolate = np.where(dems.time > times_skychop[-1])[0]
        self.assertTrue(
            (dems.d2_skychopper_isblocking.values[indices_extrapolate] == True).all(),
            "d2_skychopper_isblockingの外挿部分がTrueになることを確認",
        )

        indices_false = np.where(dems.d2_skychopper_isblocking == False)[0]
        self.assertEqual(
            indices_false[-1] + 1,
            indices_extrapolate[0],
            "外挿部分と内挿部分(Falseになっている部分)がk隣り合っていることを確認",
        )
        ...
        self.assertTrue(
            (dems.state[indices_extrapolate] == "GRAD").all(), "stateの外挿部分がGRADになることを確認"
        )
        ...
        self.assertEqual(
            indices_on[-1] + 1,
            indices_extrapolate[0],
            "stateの外挿部分と内挿部分(Falseになっている部分)が隣り合っていることを確認",
        )
    ...
    def test_merge_to_dems(self):
        ...
        with fits.open("testdata_reduced_readout.fits") as hdul:
            ...
            times = np.array(
                mf.convert_timestamp(hdul["READOUT"].data["timestamp"])
        ...
        #
        expected = np.array(
            [((15 / 88 + np.sqrt(289.15)) ** 2) / 0.5 - 2 - 288.15]
        ).astype(
            np.float32
        )  # 注意 float32
        self.assertEqual(
            round(expected[0], 4), round(dems.data[0][0], 4), "MS::dataの計算値が正しいことを確認"
        )
        ...
        self.assertEqual(
            0.5,
            round(np.count_nonzero(dems.state == "GRAD") / n_time, 1),
            "MS::stateのおよそ半数がGRADであることを確認",
        )
        self.assertEqual(
            0.5,
            round(np.count_nonzero(dems.state == "ON") / n_time, 1),
            "MS::stateのおよそ半数がONであることを確認",
        )
        ...
        #
        self.assertTrue(
            (dems.lon == -2.1 * np.cos(np.deg2rad(-2.1))).all(),
            "MS::lonの全ての計算値が正しいことを確認",
        )
        ...
        self.assertTrue(
            (dems.wind_direction == 15.0).all(), "MS::wind_directionが既定値でないことを確認"
        )
        ...
        #
        self.assertTrue(
            np.array(dems.bandwidth == 0.0).all(), "MS::bandwidthが既定値であることを確認"
        )
        self.assertTrue(
            np.array(dems.frequency == 0.0).all(), "MS::frequencyが既定値であることを確認"
        )
        self.assertTrue(
            np.array(dems.beam_major == 0.005).all(), "MS::beam_majorが既定値で無いことを確認"
        )
        self.assertTrue(
            np.array(dems.beam_minor == 0.005).all(), "MS::beam_minorが既定値で無いことを確認"
        )
        self.assertTrue(
            np.array(dems.beam_pa == 0.005).all(), "MS::beam_paが既定値で無いことを確認"
        )
        self.assertTrue(
            np.array(dems.exposure == 1 / 196).all(), "MS::exposureが既定値で無いことを確認"
        )
        self.assertTrue(
            np.array(dems.interval == 1 / 196).all(), "MS::intervalが既定値で無いことを確認"
        )

        # Observation Informations
        self.assertEqual(
            dems.observation, "saturn_zscan_v05_2_rtn_pattern6", "MS::observer"
        )
        ...
        self.assertEqual(
            dems.telescope_diameter, 10.0, "MS::telescope_diameterが既定値であることを確認"
        )

        expected = (+2230817.2140945992, -5440188.022176585, -2475718.801708271)
        self.assertEqual(
            dems.telescope_coordinates,
            expected,
            "MS::telescope_coordinatesが既定値であることを確認",
        )

        # ASTE Specific
        self.assertTrue(
            (dems.aste_cabin_temperature == 15.0 + 273.15).all(),
            "MS::aste_cabin_temperatureが既定値でないことを確認",
        )
        self.assertTrue(
            (dems.aste_misti_lon.values == 180).all(), "MS::aste_misti_lonが既定値で無いことを確認"
        )
        self.assertTrue(
            (dems.aste_misti_lat.values == 90).all(), "MS::aste_misti_latが既定値で無いことを確認"
        )
        self.assertTrue(
            (dems.aste_misti_pwv.values == 0.61).all(), "MS::aste_misti_pwvが既定値で無いことを確認"
        )
        self.assertEqual(
            "altaz", dems.aste_misti_frame, "MS::aste_misti_frameが既定値であることを確認"
        )
        ...
        self.assertTrue(
            np.array(dems.d2_mkid_frequency == 1.5).all(),
            "d2_mkid_frequencyの値を確認(DDB.KIDDES.F_filter)",
        )
        self.assertTrue(
            np.array(dems.d2_roomchopper_isblocking == False).all(),
            "d2_roomchopper_isblockingの値が既定値であることを確認",
        )
        self.assertTrue(
            np.array(dems.d2_skychopper_isblocking == False).any(),
            "d2_skychopper_isblockingの値が既定値で無いことを確認",
        )
        self.assertTrue(
            np.array(dems.d2_skychopper_isblocking == True).any(),
            "d2_skychopper_isblockingの値が既定値で無いことを確認",
        )
        self.assertEqual(
            0.5,
            round(np.count_nonzero(dems.d2_skychopper_isblocking == False) / n_time, 1),
            "MS::d2_skychopper_isblockingのおよそ半数がFalseであることを確認",
        )
        self.assertEqual(
            0.5,
    ...
    def test_retrieve_cabin_temps(self):
        ...
        datetimes, upper, lower = mf.retrieve_cabin_temps(
            "../data/deshima2.0/cosmos_20171103184436/20171103184436.cabin"
        )
        ...
        expected = np.array([datetime(2017, 11, 3, hour=18, minute=44)]).astype(
            "datetime64[ns]"
        )
        self.assertEqual(expected[0], datetimes[0])
        self.assertEqual(13.2, upper[0])
        self.assertEqual(16.6, lower[0])
        expected = np.array([datetime(2017, 11, 3, hour=18, minute=47)]).astype(
            "datetime64[ns]"
        )
    ...
    def test_retrieve_skychop_states(self):
        datetimes, states = mf.retrieve_skychop_states(
            "../data/deshima2.0/cosmos_20171103184436/20171103184436.skychop"
        )
