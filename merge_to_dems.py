...
def merge_to_dems(
    ...
    **kwargs
    ...
    times_cabin, upper_cabin_temp, lower_cabin_temp = mf.retrieve_cabin_temps(
        cabin_path
    )
    ...
    if loadtype == "Tsignal":
        # T_signalsを計算する
        master_id, kid_id, kid_type, kid_freq, kid_Q = mf.get_maskid_corresp(
            pixel_id, ddbfits_hdul
        )
        T_amb = np.nanmean(weather_table["tmperature"]) + 273.15  # 度CからKへ変換
        T_signals = mf.calibrate_to_power(
            pixel_id, lower_cabin_temp[0], T_amb, readout_hdul, ddbfits_hdul
        )
        response = T_signals
    else:
    ...
    if coordinate == "azel":
        if "az-prg(no-cor)" in antenna_table.colnames:
            ...
            raise KeyError(
                "{}ファイルにaz-prg(no-cor)列とaz-prg(no-col)列がありません。どちらか片方が存在する必要があります。ANTENNAファイルの形式を確認してください。".format(
                    antenna_path
                )
            )
    ...
    response_xr = xr.DataArray(
        data=response, dims=["time", "chan"], coords=[times, kid_id]
    )
    ...
    temperature_xr = xr.DataArray(
        data=weather_table["tmperature"], coords={"time": times_weather}
    )
    humidity_xr = xr.DataArray(
        data=weather_table["vapor-pressure"], coords={"time": times_weather}
    )
    pressure_xr = xr.DataArray(
        data=weather_table["presure"], coords={"time": times_weather}
    )
    wind_speed_xr = xr.DataArray(
        data=weather_table["aux1"], coords={"time": times_weather}
    )
    wind_direction_xr = xr.DataArray(
        data=weather_table["aux2"], coords={"time": times_weather}
    )
    skychop_state_xr = xr.DataArray(data=states_skychop, coords={"time": times_skychop})
    aste_cabin_temperature_xr = xr.DataArray(
        data=lower_cabin_temp, coords={"time": times_cabin}
    )
    ...
    state_type_numbers_xr = xr.DataArray(
        data=state_type_numbers, coords={"time": times_antenna}
    )
    ...
    if findR:
        ...
        state_right_shift = np.hstack(
            [[False] * cutnum, state_cut]
        )  # 左側をFalseで埋めて右にずらす
    ...
    return MS.new(
        ...
        exposure=1.0 / 196,  #           MergeToDfits()でも固定値が指定されていた
        interval=1.0 / 196,  #           MergeToDfits()でも固定値が指定されていた
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ddb", type=str, required=True, help="DDBファイルへのパスを指定して下さい(.fits.gz)"
    )
    parser.add_argument(
        "--obs", type=str, required=True, help="obsファイルへのパスを指定して下さい(.obs)"
    )
    parser.add_argument(
        "--antenna", type=str, required=True, help="antennaファイルへのパスを指定して下さい(.antenna)"
    )
    parser.add_argument(
        "--readout",
        type=str,
    ...
    parser.add_argument(
        "--skychop", type=str, required=True, help="skychopファイルへのパスを指定して下さい(.skychop)"
    )
    parser.add_argument(
        "--weather", type=str, required=True, help="weatherファイルへのパスを指定して下さい(.weather)"
    )
    parser.add_argument(
        "--misti", type=str, required=True, help="mistiファイルへのパスを指定して下さい(.misti)"
    )
    parser.add_argument(
        "--cabin", type=str, required=True, help="cabinファイルへのパスを指定して下さい(.cabin)"
    )

    # オプション引数
    parser.add_argument("--pixel_id", type=int, default=0, help="pixel_idを整数で指定します")
    parser.add_argument(
        "--coordinate", type=str, default="azel", help="座標系(azel/radec)を文字列で指定します"
    )
    parser.add_argument(
        "--mode",
        type=int,
        default=0,
        help="座標の読み込み方法を整数で指定します(0:相対座標cos射影あり(おすすめ既定値), 1:相対座標cos射影なし, 2:絶対座標)",
    )
    parser.add_argument(
        "--loadtype",
        type=str,
        default="Tsignal",
        help="読み込むデータを文字列で指定します(既定値: Tsignal, 現在はTsignalの読み込みしかできません)",
    )
    parser.add_argument(
        "--findR", type=bool, default=False, help="findRにTrueを指定するとFindR, Skyを実行します"
    )
    ...
    parser.add_argument(
        "--still", type=bool, default=False, help="Trueに設定するとstill観測用の解析を行います"
    )
    parser.add_argument(
        "--period", type=int, default=2, help="still観測の1/2周期(秒)を整数で指定します"
    )
    parser.add_argument(
        "--shuttle", type=bool, default=False, help="Trueを指定するとshuttle観測用の解析を行います"
    )
    parser.add_argument(
        "--lon_min_off",
        type=float,
