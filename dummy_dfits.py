...
if __name__ == "__main__":
    ...
    starttimes = [
        (now + timedelta(milliseconds=(i * 5))).isoformat() for i in range(n)
    ]  # 10分間
    ...
    n = 10 * 6  # 10秒間隔で10分間
    timestamps = [
        datetime.strftime(now + timedelta(seconds=i * 10), "%Y%m%d%H%M%S")
        for i in range(n)
    ]  # 10分間
    ...
    datetimes = [
        (now + timedelta(milliseconds=(i))).timestamp() for i in range(n)
    ]  # 10分間
    ...
    columns = [
        ...
        fits.Column(
            name="yfc, linyfc", format="2E", array=[[1.0, 1.1], [2.0, 2.2], [3.0, 3.3]]
        ),
        fits.Column(
            name="fr, dfr (300K)",
            format="2E",
            array=[[1.0, 1.1], [2.0, 2.2], [3.0, 3.3]],
        ),
        fits.Column(
            name="Qr, dQr (300K)",
            format="2E",
    ...
    timestamps = [
        (now + timedelta(milliseconds=i * 100)).isoformat() for i in range(n)
    ]  # 10分間
