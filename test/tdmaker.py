...
class TestDataMaker:
    def __init__(self, time, **kwargs):
        ...
        self.begin_time = datetime(
            year=2023, month=9, day=22, hour=9, minute=00, tzinfo=timezone.utc
        )
    ...
    def generate_all(self):
        prefix = "testdata"
        self.antenna.write(
            "{}.ant".format(prefix), format="ascii.commented_header", overwrite=True
        )
        self.skychop.write(
            "{}.skychop".format(prefix), format="ascii.commented_header", overwrite=True
        )
        self.weather.write(
            "{}.wea".format(prefix), format="ascii.commented_header", overwrite=True
        )
        self.cabin.write(
            "{}.cabin".format(prefix), format="ascii.commented_header", overwrite=True
        )
        self.misti.write(
            "{}.misti".format(prefix), format="ascii.no_header", overwrite=True
        )
    ...
    def antenna(self):
        ...
        antenna_table["time"] = [
            (
                self.begin_time + timedelta(milliseconds=self.T_antenna * 1e3 * i)
        ...
        if self.linear_antenna:
            lon_max = 180
            lon = [
                lon_max / self.measure_time * (i * self.T_antenna)
                for i in range(self.n_antenna)
            ]
    ...
    def skychop(self):
        ...
        skychop_table["ts"] = [
            (self.begin_time + timedelta(seconds=self.T_skychop * i)).timestamp()
            for i in range(self.n_skychop)
        ]
    ...
    def weather(self):
        ...
        weather_table["time"] = [
            (self.begin_time + timedelta(seconds=self.T_weather * i)).strftime(
                "%Y%m%d%H%M%S"
            )
            for i in range(self.n_weather)
        ]
    ...
    def misti(self):
        ...
        misti_table["date"] = [
            (self.begin_time + timedelta(seconds=self.T_misti * i)).strftime("%Y/%m/%d")
            for i in range(self.n_misti)
        ]
        misti_table["time"] = [
            (self.begin_time + timedelta(seconds=self.T_misti * i)).strftime(
                "%H:%M:%S.%f"
            )
            for i in range(self.n_misti)
        ]
    ...
    def cabin(self):
        ...
        cabin_table["date"] = [
            (self.begin_time + timedelta(seconds=self.T_cabin * i)).strftime("%Y/%m/%d")
            for i in range(self.n_cabin)
        ]
        cabin_table["time"] = [
            (self.begin_time + timedelta(seconds=self.T_cabin * i)).strftime("%H:%M")
            for i in range(self.n_cabin)
        ]
    ...
    def ddb(self):
        ...
        columns = [
            ...
            fits.Column(
                name="masterid", format="I", array=[1 for i in range(self.n_kid)]
            ),
            fits.Column(
                name="F_filter, dF_filter", format="2E", array=[dummy] * self.n_kid
            ),
            fits.Column(
                name="Q_filter, dQ_filter", format="2E", array=[dummy] * self.n_kid
            ),
        ]
    ...
    def readout(self, **kwargs):
        ...
        columns = [
            ...
            fits.Column(name="pixelid", format="I", array=[0] * self.n_kid),
            fits.Column(
                name="yfc, linyfc", format="2E", array=[(1.0, self.linyfc)] * self.n_kid
            ),
            fits.Column(name="fr, dfr (300K)", format="2E", array=[dummy] * self.n_kid),
            fits.Column(
                name="Qr, dQr (300K)", format="2E", array=[(self.Qr, 0.25)] * self.n_kid
            ),
        ...
        columns = [
            fits.Column(
                ...
                array=[
                    (
                        self.begin_time
                        + timedelta(microseconds=self.T_readout * 1e6 * i)
                    ).timestamp()
        ...
        if self.linear_readout == "inc":
            ...
            dummy = [
                (1.0, 1.0, linPh * np.sqrt(i * self.T_readout))
                for i in range(self.n_readout)
            ]
            ...
            dummy = [
                (
                    ...
                    1.0,
                    np.sqrt(
                        -linPh_max / self.readout_time * i * self.T_readout + linPh_max
                    ),
                )
                for i in range(self.n_readout)
            ]
    ...
    def dfits(self):
        ...
        columns = [
            fits.Column(
                name="masterids", format="63K", array=[i for i in range(self.n_kid)]
            ),
            fits.Column(
                name="kidids", format="63K", array=[i for i in range(self.n_kid)]
            ),
        ...
        columns = [
            fits.Column(
                ...
                array=[
                    (
                        self.begin_time
                        + timedelta(milliseconds=self.T_antenna * 1e3 * i)
                    ).isoformat()
                    for i in range(self.n_antenna)
                ],
            ),
        ...
        columns = [
            fits.Column(
                ...
                array=[
                    (
                        self.begin_time + timedelta(seconds=self.T_weather * i)
                    ).isoformat()
                    for i in range(self.n_weather)
                ],
            ),
            ...
            fits.Column(
                name="vapor-pressure", format="D", array=[0.5] * self.n_weather
            ),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--linyfc", type=float, default=0.25, help="linyfcをfloatで指定して下さい"
    )
    parser.add_argument(
        "--linear_readout",
        type=str,
        default="",
        help="readoutの値を線形に変化させる場合はinc/decのいずれかを指定して下さい",
    )
    parser.add_argument(
        "--linear_antenna",
        type=bool,
        default=False,
        help="antennaのlonを線形に変化させる場合はTrueを指定して下さい",
    )
    parser.add_argument(
        "--all_grad", type=bool, default=False, help="すべてのSCAN状態をGRADにする場合はTrueを指定して下さい"
    )
    parser.add_argument(
        "--lower_cabin_temp",
        type=float,
        default=15,
        help="MainCabinの温度(degC)をfloatで指定して下さい",
    )
    parser.add_argument(
        "--prefix", type=str, default="testdata", help="生成されるファイル名のprefixを指定して下さい"
    )
    parser.add_argument(
        "--measure_time", type=int, default=None, help="環境測定時間(分)を整数で指定して下さい"
    )
    a = parser.parse_args()
    ...
    if a.data_name == "antenna":
        tdm.antenna.write(
            "{}.ant".format(a.prefix), format="ascii.commented_header", overwrite=True
        )
        sys.exit(0)
    if a.data_name == "skychop":
        tdm.skychop.write(
            "{}.skychop".format(a.prefix),
    ...
    if a.data_name == "weather":
        tdm.weather.write(
            "{}.wea".format(a.prefix), format="ascii.commented_header", overwrite=True
        )
        sys.exit(0)
    if a.data_name == "misti":
        tdm.misti.write(
            "{}.misti".format(a.prefix), format="ascii.no_header", overwrite=True
        )
        sys.exit(0)
    if a.data_name == "cabin":
        tdm.cabin.write(
            "{}.cabin".format(a.prefix), format="ascii.commented_header", overwrite=True
        )
        sys.exit(0)
    if a.data_name == "ddb":
