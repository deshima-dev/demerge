...
def make_reduced_fits(kidfiles, output_filename):
    fitinfo = (
        []
    )  # [[kidid, pread, fc, yfc, linyfc, fr, dfr, Qr, dQr, Qc, dQc, Qi, dQi], ...]
    ...
    for kidfile in kidfiles:
        with open(kidfile, "rb") as f:
            kid = pickle.load(f)
        timestamp = kid["tod_timestamp"][
            :-100
        ]  # データの最後の部分にはよくないデータが含まれる可能性があるので、最後の100個ぐらいを除外する。
        ts = dmerge.rebin_array(
            timestamp, downsampling_rate
        )  # 1はdown samplingしないことを意味する
        ...
        if kid["resonance_params"] is None:
            ...
            rewound_tod = dmerge.FixedFitData(
            ...
            d = dmerge.FixedFitData(
                rewound_tod.frequency * GHz, rewound_tod.t, ampl, phase
            )
        ...
        readout["hdr_com_lis"].append(
            "data format of field {}".format(kid["kidid"] + 2)
        )
        continue
    ...
    kidid_, pread_, fc_, yfc_, linyfc_, fr_, dfr_, Qr_, dQr_, Qc_, dQc_, Qi_, dQi_ = [
        np.array(fitinfo)[:, i] for i in range(13)
    ]
