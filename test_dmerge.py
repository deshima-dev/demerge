...
class DmergeTestDrive(unittest.TestCase):
    ...
    def load_data(self):
        """SweepデータやTODを読み込む"""
        kidslist = dmerge.load_kidslist('../raw_dataset/obs/cosmos_20171103184836/kids.list')
        localsweeps = dmerge.load_localsweep('../raw_dataset/obs/cosmos_20171103184836/localsweep.sweep', framelen=kidslist[0]['framelen'])
        tods = dmerge.TODs('../raw_dataset/obs/cosmos_20171103184836/20171103184836.fits')
        return kidslist, localsweeps, tods

    def test_SweepData(self):
        """SweepDataクラスのテスト"""
        sweep = dmerge_deprecated._read_sweep('../raw_dataset/obs/cosmos_20171103184836/localsweep.sweep')
    ...
    def test_FixedData(self):
        ...
        if os.path.exists('cache'):
            shutil.rmtree('cache')
        cmd = ' '.join(['python make_divided_data.py',
        ...
        expected = FixedData('I-Q-Gain', (tods.timestamp[:tods.offset],), (read_I_, read_Q_, tods.fftgain), tods.carrier_freq[index], info=info)
    ...
    def test_FixedFitData(self):
        ...
        expected = FixedFitData(kid['tod'].frequency, kid['tod'].t, ampl, phase, info=kid['tod'].info)
        result = dmerge.FixedFitData(kid['tod'].frequency, kid['tod'].t, ampl, phase, kid['tod'].info)
    ...
    def test_kids_with_both_blinds(self):
        ...
        kidslist = dmerge.load_kidslist('../raw_dataset/obs/cosmos_20171103184836/kids.list')
        expected = dmerge_deprecated.kids_with_both_blinds(kidslist, allow_without_blind=True)
    ...
    def test_read_kidslist(self):
        ...
        expected = dmerge_deprecated.read_kidslist('../raw_dataset/obs/cosmos_20171103184836/kids.list')
        info, kids, blinds, powers = expected
        result = dmerge.load_kidslist('../raw_dataset/obs/cosmos_20171103184836/kids.list')
    ...
    def test_load_localsweep(self):
        ...
        info, kids, blinds, powers = dmerge.load_kidslist('../raw_dataset/obs/cosmos_20171103184836/kids.list')
    ...
    def test_fit_sweep(self):
        ...
        expected = np.load('workspace/analysis/ana_data/cosmos_20171103184836_01011_ana/FitSweep_fit.npy')
        ...
        for kidid in range(len(kids)):
            with self.subTest(i=kidid):
                ...
                Q_search=100
                peaks = dmerge.find_peaks(kids[kidid]['localsweep'].x, kids[kidid]['localsweep'].amplitude, fc, minq=Q_search, maxratio=1)
                try:
                    result, fitrange = dmerge.fit_onepeak(kids[kidid]['localsweep'], peaks, nfwhm=5)
                except Exception:
                    offset += 1
                    continue
    ...
    def test_parallel_calc_resonance_params(self):
        ...
        cmd = ' '.join(['python make_divided_data.py',
                        ...
                        'cache'])
        os.system(cmd)
        cmd = ' '.join(['ls cache/*.pkl | xargs -P7 -n1',
                        'python',
                        'calc_resonance_params.py'])
        os.system(cmd)
    ...
    def test_rewind(self):
        ...
        for kidid in range(len(glob.glob('cache/kid*.pkl'))):
            ...
            peaks = dmerge.find_peaks(kid['localsweep'].x, kid['localsweep'].amplitude, fc, minq=100, maxratio=1)
            kidfitresult, fitrange = dmerge.fit_onepeak(kid['localsweep'], peaks, nfwhm=5)
            ...
            filename = '../raw_dataset/obs/cosmos_20171103184836/reduced_20171103184836.fits'
    ...
    def test_make_reduced_fits(self):
        ...
        cmd = ' '.join(['python make_reduced_fits.py',
                        'cache',
                        'cache/reduced.fits'])
        os.system(cmd)
        self.assertTrue(os.path.exists('cache/reduced.fits'))
        hdul_expected = fits.open('../raw_dataset/obs/cosmos_20171103184836/reduced_20171103184836.fits', memmap=False)
        hdul_result = fits.open('cache/reduced.fits', memmap=False)

        extname = 'KIDSINFO'
        fields = ['pixelid', 'kidid', 'Pread', 'fc', 'yfc, linyfc', 'fr, dfr (300K)', 'Qr, dQr (300K)', 'Qc, dQc (300K)', 'Qi, dQi (300K)']
        for field in fields:
            j = 0
                ...
                with self.subTest(i=field, j=j):
                    if type(expected) == np.ndarray:
                        if np.isnan(expected[0]):
                            self.assertTrue(np.isnan(hdul_result[extname].data[field][j][0]))
                        else:
                            self.assertEqual(hdul_result[extname].data[field][j][0], expected[0])

                        if np.isnan(expected[1]):
                            self.assertTrue(np.isnan(hdul_result[extname].data[field][j][1]))
                        else:
                            self.assertEqual(hdul_result[extname].data[field][j][1], expected[1])
        ...
        for field in fields:
            ...
            for expected in hdul_expected[extname].data[field]:
                with self.subTest(i=field, j=j):
                    if np.isnan(expected[0]):
                        self.assertTrue(np.isnan(hdul_result[extname].data[field][j][0]), 'Amp is nan') 
                        self.assertTrue(np.isnan(hdul_result[extname].data[field][j][1]), 'Ph is nan') 
                        self.assertTrue(np.isnan(hdul_result[extname].data[field][j][2]), 'linPh is nan') 
                    else:         
                        self.assertEqual(hdul_result[extname].data[field][j][0], expected[0], 'Amp') 
                        self.assertEqual(hdul_result[extname].data[field][j][1], expected[1], 'Ph') 
                        self.assertEqual(hdul_result[extname].data[field][j][2], expected[2], 'linPh') 
                j += 1
    ...
    def test_parallel_plot_sweep(self):
        ...
        cmd = ' '.join(['echo "', s , '"',
                        ...
                        'plot_sweep.py'])
        os.system(cmd)
        self.assertEqual(len(os.listdir('graphs')), 2*(63 - 1))#63個のKIDの内46番だけフィットできていないのでプロットされない。
        return
